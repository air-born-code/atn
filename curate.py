...
from curate_tables import PICKS, LIVE, SONGS, EXCLUDE, listen_links
...
CSCORES, CBEATS = COMMITTEE["scores"], COMMITTEE["beats"]
...
for r in raw:
    ...
    cs = CSCORES.get(a) or {}
    beats = CBEATS.get(DAY_KEY[r["day"]] + "|" + a, [])
    sets.append({
        "cs": cs.get("c", 0),          # committee composite
        "ch": cs.get("h", 0),          # heritage
        "cl": cs.get("l", 0),          # legendariness
        "cr": cs.get("r", 0),          # recent live form
        "cf": cs.get("cf", 0),         # confidence in the recent figure
        "bt": beats,                   # who the committee says this beats
        "sg": song,
